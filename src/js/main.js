...
const calculator = () => {
	...
	document.documentElement.addEventListener('click', function(e){
		...
		if(target.closest('.js-borderColor') && !target.closest('.select__item')){
			document.querySelectorAll('.js-borderColor').forEach(item => {
				item.classList.remove('is-borderColorBlue')
			})
			target.closest('.js-borderColor').classList.add('is-borderColorBlue')
		} else {
			document.querySelectorAll('.js-borderColor').forEach(item => {
				item.classList.remove('is-borderColorBlue')
			})
	...
	function formula(){
		...
		resultSum.innerHTML = Math.round(sumDouble.value.replace(/\D/g,'') * k).toLocaleString('ru'); 
	}
	formula()
	...
	sum.addEventListener('input', function() {
		...
		if(sumDouble.value >= Number(sumDouble.dataset.max)){
			...
			this.value = Number(sumDouble.dataset.max.replace(/\D/g,'')).toLocaleString('ru')
